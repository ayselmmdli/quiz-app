...
function showQuestion(index){
    ...
    let optionTag = `<div class="option"><span>${questions[index].choices[0]} </span></div>
...
function optionSelected(answ){
    ...
    if(userAnswer.includes(correctAnswer)){
        ...
        for(let i = 0 ; i < allOptions ; i++){
            if(questionList.children[i].textContent.includes(correctAnswer)){
...
function showResultBox(){
    ...
    let progress = setInterval(() => {
        ...
        circularProgress.style.background = `conic-gradient(#6e44fe ${progressStartValue * 3.6}deg, rgba(0,0,0,.1) 0deg)`
        // console.log(progressStartValue)
